#include <catch2/catch_test_macros.hpp>

#include "aoc/days/Day1.hpp"
#include "aoc/days/Day10.hpp"
#include "aoc/days/Day11.hpp"
#include "aoc/days/Day12.hpp"
#include "aoc/days/Day2.hpp"
#include "aoc/days/Day3.hpp"
#include "aoc/days/Day4.hpp"
#include "aoc/days/Day5.hpp"
#include "aoc/days/Day6.hpp"
#include "aoc/days/Day7.hpp"
#include "aoc/days/Day8.hpp"
#include "aoc/days/Day9.hpp"

using namespace aoc2025;

TEST_CASE("Day 1 regression tests", "[Regression]") {
    Day1 d;
    d.parse();
    REQUIRE(std::get<uint64_t>(d.part1()) == 1048);
    REQUIRE(std::get<uint64_t>(d.part2()) == 6498);
}
TEST_CASE("Day 2 regression tests", "[Regression]") {
    Day2 d;
    d.parse();
    REQUIRE(std::get<uint64_t>(d.part1()) == 28846518423ull);
    REQUIRE(std::get<uint64_t>(d.part2()) == 31578210022ull);
}

TEST_CASE("Day 3 regression tests", "[Regression]") {
    Day3 d;
    d.parse();
    REQUIRE(std::get<uint64_t>(d.part1()) == 17535);
    REQUIRE(std::get<uint64_t>(d.part2()) == 173577199527257ull);
}

TEST_CASE("Day 4 regression tests", "[Regression]") {
    Day4 d;
    d.parse();
    REQUIRE(std::get<uint64_t>(d.part1()) == 1564);
    REQUIRE(std::get<uint64_t>(d.part2()) == 9401);
}

TEST_CASE("Day 5 regression tests", "[Regression]") {
    Day5 d;
    d.parse();
    REQUIRE(std::get<uint64_t>(d.part1()) == 811);
    REQUIRE(std::get<uint64_t>(d.part2()) == 338189277144473ull);
}

TEST_CASE("Day 6 regression tests", "[Regression]") {
    Day6 d;
    d.parse();
    REQUIRE(std::get<uint64_t>(d.part1()) == 5335495999141ull);
    REQUIRE(std::get<uint64_t>(d.part2()) == 10142723156431ull);
}

TEST_CASE("Day 7 regression tests", "[Regression]") {
    Day7 d;
    d.parse();
    REQUIRE(std::get<uint64_t>(d.part1()) == 1555);
    REQUIRE(std::get<uint64_t>(d.part2()) == 12895232295789ull);
}

TEST_CASE("Day 8 regression tests", "[Regression]") {
    Day8 d;
    d.parse();
    REQUIRE(std::get<uint64_t>(d.part1()) == 181584);
    REQUIRE(std::get<uint64_t>(d.part2()) == 8465902405);
}

TEST_CASE("Day 9 regression test", "[Regression]") {
    Day9 d;
    d.parse();

    REQUIRE(std::get<uint64_t>(d.part1()) == 4781377701);
    REQUIRE(std::get<uint64_t>(d.part2()) == 1470616992);
}

TEST_CASE("Day 10 regression test", "[Regression]") {
    Day10 d;
    d.parse();
    REQUIRE(std::get<uint64_t>(d.part1()) == 517);
    REQUIRE(std::get<uint64_t>(d.part2()) == 21469);
}

TEST_CASE("Day 11 regression test", "[Regression]") {
    Day11 d;
    d.parse();
    REQUIRE(std::get<uint64_t>(d.part1()) == 772);
    REQUIRE(std::get<uint64_t>(d.part2()) == 423227545768872ull);
}

TEST_CASE("Day 12 regression test", "[Regression]") {
    Day12 d;
    d.parse();
    REQUIRE(std::get<uint64_t>(d.part1()) == 577);
}
